...
def upload_file():
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            file.save(data)
            # ifile = open(file, 'w')
            #datan = csv.reader(file, delimiter=',')
            print(data)
...
def run_data(src):
    ...
    print("Test")
    #return 'endeneu'
    return send_file('results/fileUpload/result_sleep_prediction.csv')
...
def run_demo(testvariable):
    ...
    # return testvariable
    return send_file('results/inputAccelero/result_sleep_prediction.csv')
    # return 'neuerReturn'
